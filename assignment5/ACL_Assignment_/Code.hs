...
-}
acl_size = 5                                         -- table size
acl_tbl  = Dynamic("acl", (acl_size, ["srcmac"],["dstmac"]))       -- dynamic table "acl"
acl_val  = Static([[("srcmac", 0)] | x <- [1..acl_size]],[[("dstmac", 0)] | y <- [1..acl_size]]) -- default values      
acl_ptrn = [("srcmac", "dstmac")]                          -- pattern for loading the modify table
