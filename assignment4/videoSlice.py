...
class VideoSlice (EventMixin):
    ...
    def __init__(self):
        ...
        #egress_port = self.adjacency[this_dpid][next_switch]
                ...
                try:
                    ...
                    if tcpp:
                        ...
                        if (tcpp.srcport == 80 or tcpp.dstport == 80):
                            if (this_dpid == '00-00-00-00-00-01' and (dstmac == EthAddr('00:00:00:00:00:01'))):
                                outport = 3
                            elif (this_dpid == '00-00-00-00-00-01' and (dstmac == EthAddr('00:00:00:00:00:02'))):
                                outport = 4
                            elif (this_dpid == '00-00-00-00-00-04' and (dstmac == EthAddr('00:00:00:00:00:03'))):
                                outport = 3
                            elif (this_dpid == '00-00-00-00-00-04' and (dstmac == EthAddr('00:00:00:00:00:04'))):
                                outport = 4
                            else:
                                next_switch = self.portmap1[(this_dpid,srcmac,dstmac,80)]
                                print "-----> next switch: ", next_switch
                                outport = self.adjacency[this_dpid][next_switch]
                                print '-------> outport:',outport
                            install_fwdrule(event,packet,outport)

                    
                    """
                    check = (this_dpid, packet.src, packet.dst, packet.find('tcp').dstport)
                    
                    if not self.portmap.get(check):
                        check = (this_dpid, packet.src, packet.dst, packet.find('tcp').srcport)
                        
                        if not self.portmap.get(check):
                            raise AttributeError

                    new_dpid = self.portmap[check]
                    ...
                    """
                    # learn position for destination ethrnet address
                    dest_loc=self.portmap[dpid_to_str(event.dpid),EthAddr(dpid_to_str(event.dpid)),EthAddr(packet.dst),event.port]

                    log.debug("destination PORT :----> %s :",dest_loc)

                    if dest_loc==('00-00-00-00-00-03'):
                        self.install_fwdrule(event,packet,2)
                        
                    elif dest_loc==('00-00-00-00-00-02'):
                        self.install_fwdrule(event,packet,1)
                        
                    elif dest_loc==('00-00-00-00-00-04'):
                        self.install_fwdrule(event,packet,2)
                    """
                    """    
